...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
    ...
    // AUV2000 Custom Message Branching
    case MAVLINK_MSG_ID_VIAM_PISTON_STATUS:
        _handlePiston(message);
        break;
    case MAVLINK_MSG_ID_VIAM_MASS_SHIFTER_STATUS:
        _handleMassShifter(message);
        break;
    case MAVLINK_MSG_ID_VIAM_THRUSTER_STATUS:
        _handleThruster(message);
        break;
    case MAVLINK_MSG_ID_VIAM_BOARD_ARM1_STATUS:
        _handleBoardARM1(message);
        break;
    case MAVLINK_MSG_ID_VIAM_BOARD_ARM2_STATUS:
        _handleBoardARM2(message);
        break;
...
void Vehicle:: _handleMassShifter(mavlink_message_t& message)
{
    mavlink_viam_mass_shifter_status_t mass_shifter;
    mavlink_msg_viam_mass_shifter_status_decode(&message, &mass_shifter);

    _mass_shifterPosition = mass_shifter.position;
    emit mass_shifterPositionChanged(_mass_shifterPosition);

    _mass_shifterDuty = mass_shifter.motor_duty;
    emit mass_shifterDutyChanged(_mass_shifterDuty);

    _mass_shifterDSpeed = mass_shifter.motor_dspeed;
     emit mass_shifterDSpeedChanged(_mass_shifterDSpeed);

    _mass_shifterRSpeed = mass_shifter.motor_rspeed;
     emit mass_shifterRSpeedChanged(_mass_shifterRSpeed);

    _mass_shifterCurrent = mass_shifter.motor_current;
     emit mass_shifterCurrentChanged(_mass_shifterCurrent);

    _mass_shifterTempOnChip = mass_shifter.motor_temp_on_chip;
     emit mass_shifterTempOnChipChanged(_mass_shifterTempOnChip);

    _mass_shifterTempAmbient = mass_shifter.motor_temp_ambient;
     emit mass_shifterTempAmbientChanged(_mass_shifterTempAmbient);
}

void Vehicle::_handlePiston(mavlink_message_t& message)
{
    mavlink_viam_piston_status_t piston;
    mavlink_msg_viam_piston_status_decode(&message, &piston);

    _pistonPosition = piston.position;
    emit pistonPositionChanged(_pistonPosition);

    _pistonDuty = piston.motor_duty;
    emit pistonDutyChanged(_pistonDuty);

    _pistonDSpeed = piston.motor_dspeed;
     emit pistonDSpeedChanged(_pistonDSpeed);

    _pistonRSpeed = piston.motor_rspeed;
     emit pistonRSpeedChanged(_pistonRSpeed);

    _pistonCurrent = piston.motor_current;
     emit pistonCurrentChanged(_pistonCurrent);

    _pistonTempOnChip = piston.motor_temp_on_chip;
     emit pistonTempOnChipChanged(_pistonTempOnChip);

    _pistonTempAmbient = piston.motor_temp_ambient;
     emit pistonTempAmbientChanged(_pistonTempAmbient);
}

void Vehicle::_handleThruster(mavlink_message_t& message)
{
    mavlink_viam_thruster_status_t thruster;
    mavlink_msg_viam_thruster_status_decode(&message, &thruster);

    _thrusterDuty = thruster.motor_duty;
    emit thrusterDutyChanged(_thrusterDuty);

    _thrusterDSpeed = thruster.motor_dspeed;
     emit thrusterDSpeedChanged(_thrusterDSpeed);

    _thrusterRSpeed = thruster.motor_rspeed;
     emit thrusterRSpeedChanged(_thrusterRSpeed);

    _thrusterCurrent = thruster.motor_current;
     emit thrusterCurrentChanged(_thrusterCurrent);

    _thrusterTempOnChip = thruster.motor_temp_on_chip;
     emit thrusterTempOnChipChanged(_thrusterTempOnChip);

    _thrusterTempAmbient = thruster.motor_temp_ambient;
     emit thrusterTempAmbientChanged(_thrusterTempAmbient);
}

void Vehicle::_handleBoardARM1(mavlink_message_t& message)
{
    mavlink_viam_board_arm1_status_t board_arm1;
    mavlink_msg_viam_board_arm1_status_decode(&message, &board_arm1);

    _pistonLSHead = board_arm1.ls_piston_athead;
    emit pistonLSHeadChanged(_pistonLSHead);

    _pistonLSTail = board_arm1.ls_piston_attail;
    emit pistonLSTailChanged(_pistonLSTail);
...
void Vehicle::_handleBoardARM2(mavlink_message_t& message)
{
    mavlink_viam_board_arm2_status_t board_arm2;
    mavlink_msg_viam_board_arm2_status_decode(&message, &board_arm2);

    _rudderLoad = board_arm2.rudder_load;
    emit rudderLoadChanged(_rudderLoad);

    _rudderSpeed = board_arm2.rudder_speed;
    emit rudderSpeedChanged(_rudderSpeed);

    _rudderVoltage = board_arm2.rudder_voltage;
    emit rudderVoltageChanged(_rudderVoltage);

    _rudderPosition = board_arm2.rudder_position;
    emit rudderPositionChanged(_rudderPosition);

    _rudderTemperature = board_arm2.rudder_temperature;
    emit rudderTemperatureChanged(_rudderTemperature);
}
